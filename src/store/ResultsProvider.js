...
const ResultsProvider = (props) => {
  ...
  const setResultsHandler = () => {
      ...
      for (let x = 0; x < data.after * 12; x++) {
        if (x === 0) {
          ...
          item = {
            ...
            endBalance:
              beginningInterest(holder[x - 1].endBalance + contribution, r, t) +
      ...
      for (let x = 0; x < data.after * 12; x++) {
        ...
        if (x === 0) {
          item = {
            ...
            endBalance: endInterest(principal, r, t) + contribution + principal,
            endPrincipal: principal + contribution,
          };
        } else {
          item = {
    ...
    resultsState.results.endBalance =
      resultsState.results.monthlyResults[
        Number(inputsCtx.after * 12 - 1)
      ].endBalance;
  };
